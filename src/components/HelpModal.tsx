...
const SECTIONS = [
  {
    ...
    items: [
      ...
      "**MIXER** mode — master bus EQ, glue comp, drive, and limiter",
      "Switch modes with the **PAD | LOOPER | MIXER** toggle in the header",
  ...
  },
  {
    ...
    items: [
      ...
      "**10 grid effects**: Delay, Distortion, Reverb, Compressor, Flanger, Duck, Chorus, Phaser, Bitcrusher, Tremolo (LPF & HPF hidden — driven by the XY pad)",
      "**Waveform background** — the pad canvas shows a live oscilloscope of the master output behind the grid",
  ...
  },
  {
    title: "Master Mixer",
    items: [
      "**MIXER** mode — pro-style master bus chain applied to everything: PAD hits, loops, and resamples",
      "Signal flow (left to right): **HPF → LOW → MID → HIGH → GLUE → DRIVE → LIMITER → VOL**",
      "**HPF** — highpass filter (rumble cut): cycles **OFF / 20 / 30 / 40 Hz**. Removes sub-low noise that eats headroom",
      "**3-band EQ** — LOW (shelf @ 250 Hz), MID (peaking @ 1 kHz, Q=1), HIGH (shelf @ 4 kHz); each ±18 dB with a centre tick at 0",
      "**GLUE** — gentle 2:1 bus compressor with a single AMOUNT knob; 0 = bypass, 1 = heavy glue (auto makeup gain)",
      "**DRIVE** — tanh soft-clip waveshaper (1×–10×) with auto loudness compensation so turning it up doesn't just get louder",
      "**LIMITER** — brick-wall limiter with **ON/OFF** toggle, adjustable **CEIL** (output ceiling, -6 to 0 dB, default -1 dB), and a live **CLIP** LED that lights red on peak hits",
      "**VOL** — final output trim (post-limiter). The header VOL slider also targets this node",
      "The header **VOL** slider and the mixer VOL fader are the same control — always the last stage in the chain",
    ],
  },
  {
    title: "Timing & Sync",
    items: [
      "**SNAP** — toggle quantized (bar-snapped) recording on/off; dim = free timing, lit = snap on",
  ...
  },
  {
    ...
    items: [
      ...
      "**⚙** Settings — 6 themes (3 dark + 3 light), audio input device, session export/import, recording limits",
