/**
 * BpmControl — stacked ▲▼ arrows next to a clickable BPM display.
 *
 * Ported from mpump's BpmControl so the two apps share the same
 * tempo affordance. Clicking the number opens a modal with a
 * numeric input; holding an arrow accelerates stepping.
 */
...
export function BpmControl({ bpm, command }: Props) {
  ...
  // Keep bpmRef current for the accelerated hold-to-step handlers. Assigning
  // during render trips react-hooks/refs; an effect is the sanctioned spot.
  useEffect(() => {
    bpmRef.current = bpm;
  }, [bpm]);
  ...
  const step = useCallback(
    (delta: number) => {
      holdCount.current++;
      // After ~15 ticks, accelerate to ±5 per step for faster sweeps
        ...
        <div className="bpm-modal-overlay" onClick={closeModal}>
          <div className="bpm-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bpm-modal-actions">
              ...
              <button
                className="bpm-modal-btn bpm-modal-btn-ok"
                onClick={() => applyBpm(inputRef.current?.value ?? "")}
              >Set</button>
            </div>
          </div>
        </div>
